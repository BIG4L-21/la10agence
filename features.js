...
document.addEventListener('DOMContentLoaded', () => {
  ...
  ════════════════════════════════════════════════════════ */
  const slider   = document.getElementById('tem-slider');
  const prevBtn  = document.getElementById('tem-prev');
  const nextBtn  = document.getElementById('tem-next');
  const dotsEl   = document.getElementById('tem-dots');

  if (slider && prevBtn && nextBtn) {
    const cards      = slider.querySelectorAll('.tem-card');
    ...
    for (let i = 0; i < numDots; i++) {
      ...
      dot.className = 'tem-dot' + (i === 0 ? ' active' : '');
    ...
    function goTo(idx) {
      ...
      const cardWidth = cards[0].offsetWidth + 16; // gap = 1rem = 16px
      ...
      // Update dots
      dotsEl.querySelectorAll('.tem-dot').forEach((d, i) => {
