...
/* ══════════════════════════════════════════════════════════
   MEDIA MANAGER
══════════════════════════════════════════════════════════ */
// Store uploaded media URLs in Supabase textes table
const MEDIA_KEY = 'admin_media_list';
...
async function loadMedias() {
  ...
  try {
    const data = await api.get('textes', `id=eq.${MEDIA_KEY}`);
    const items = data[0] ? JSON.parse(data[0].valeur) : [];
    ...
    grid.innerHTML = items.map((item, i) => `
      <div class="media-item">
        <img src="${item.url}" alt="${item.name}"
             onerror="this.src='data:image/svg+xml,<svg xmlns=%22http://www.w3.org/2000/svg%22 width=%22160%22 height=%22120%22><rect fill=%22%23222%22 width=%22160%22 height=%22120%22/><text fill=%22%23555%22 x=%2250%25%22 y=%2250%25%22 text-anchor=%22middle%22 dominant-baseline=%22middle%22 font-size=%2212%22>Image</text></svg>'"/>
        <div class="media-item-info">
          <div class="media-item-name">${item.name}</div>
          <div class="media-item-size">${item.size || ''}</div>
        </div>
        <div class="media-item-actions">
          <button class="media-action-btn" onclick="copyMediaUrl('${item.url}')" title="Copier URL">📋</button>
          <button class="media-action-btn" onclick="deleteMedia(${i})" title="Supprimer">🗑</button>
        </div>
      </div>`).join('');
  } catch(e) {
...
async function saveMediaList(items) {
  await api.upsert('textes', [{
    id: MEDIA_KEY,
    valeur: JSON.stringify(items),
    updated_at: new Date().toISOString()
  }]);
}
...
async function getMediaList() {
  try {
    const data = await api.get('textes', `id=eq.${MEDIA_KEY}`);
    return data[0] ? JSON.parse(data[0].valeur) : [];
  } catch { return []; }
}

// Convert file to base64 and store as data URL
// (For real hosting, you'd use Supabase Storage or Cloudinary)
async function uploadMedia(file) {
  return new Promise((resolve, reject) => {
    if (file.size > 5 * 1024 * 1024) {
      reject(new Error('Fichier trop grand (max 5MB)'));
      return;
    }
...
function setupMediaUpload() {
  ...
  async function handleUpload(files) {
    ...
    status.textContent = `Upload de ${files.length} fichier(s)...`;
